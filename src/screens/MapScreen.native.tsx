...
export const MapScreen: React.FC = () => {
  ...
  useEffect(() => { 
    (async () => { 
      try { 
        ...
        setRegion({ latitude: coords.latitude, longitude: coords.longitude, latitudeDelta: 0.02, longitudeDelta: 0.02 }); 
        mapRef.current?.animateToRegion({ latitude: coords.latitude, longitude: coords.longitude, latitudeDelta: 0.02, longitudeDelta: 0.02 }, 500); 
  ...
  return (
    <View style={styles.container}>
      ...
      <CenterButton onPress={async () => { 
        try { 
          const coords = await getCurrentLocationAsync(); 
          mapRef.current?.animateToRegion({ latitude: coords.latitude, longitude: coords.longitude, latitudeDelta: 0.02, longitudeDelta: 0.02 }, 400); 
        } catch (e) { 
